...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\linewidth]{media/circuit-boost-converter}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\linewidth]{media/mode1-boostR}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\linewidth]{media/mode2-boostR}
...
El principal cambio para el elevador del voltaje es el añadido del capacitor en la etapa de salida, esto con la finalidad de reducir el voltaje de rizo (\textit{voltage ripples}), siendo así que su comportamiento se puede estudiar a partir de su ecuación diferencial como se muestra en \cite{hart_power_electronics}
...
Mediante tales expresiones es posible hacer uso de la ecuación \ref{eq:deltai-general} para obtener la inductancia crítica y cuyo valor debe ser mayor igual al obtenido a la expresión definida en \ref{eq:lc-boostR}
...
La condición crítica para la elección de la capacitancia límite es el voltaje de salida promedio $V_a$ cuyo valor se duplicará para la variación en el voltaje de capacitor \textit{voltaje rizo}, mediante las siguientes expresiones.
...
Y del cual se puede determinar que mediante una relación para el voltaje rizo se requiere que este valor sea pequeño o no cuente con muchas fluctuaciones siendo así que se establece una relación mayor igual en relación a los parámetros definidos en \ref{eq:cap-critica}.

\begin{equation}
	C_C = \frac{K}{2FR}
	\label{eq:cap-critica}
\end{equation}

Obteniendo finalmente que la capacitancia crítica tiene que ser mayor al valor definido por el ciclo de trabajo respecto a la frecuencia y carga (R) de salida.
